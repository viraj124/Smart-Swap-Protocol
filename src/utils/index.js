...
import NFT_ABI from '../constants/abis/nft'
import NFT_MARKET_ABI from '../constants/abis/nftmarket'
import ERC20_ABI from '../constants/abis/erc20'
import CTOKEN_ABI from '../constants/abis/ctoken'
import DAIABI from '../constants/abis/dai'
...
export function getEtherscanLink(networkId, data, type) {
  const prefix = `https://explorer.testnet2.matic.network`
...
export function getNFTMarketContract(nftMarketAddress, library, account) {
  return getContract(nftMarketAddress, NFT_MARKET_ABI, library, account)
}

export function getNFTContract(nftaAddress, library, account) {
  return getContract(nftaAddress, NFT_ABI, library, account)
}

export function getCtokenContract(cdaiAddress, library, account) {
  return getContract(cdaiAddress, CTOKEN_ABI, library, account)
}

export function getDaiContract(daiAddress, library, account) {
  return getContract(daiAddress, DAIABI, library, account)
}
...
// get the nft token balance of an address
export async function getNFTTokenBalance(tokenAddress, address, library) {
  if (!isAddress(tokenAddress) || !isAddress(address)) {
    throw Error(`Invalid 'tokenAddress' or 'address' parameter '${tokenAddress}' or '${address}'.`)
  }

  return getContract(tokenAddress, NFT_ABI, library).balanceOf(address)
}
...
export async function getAccountSnapshot(address, ctokenaddress, library) {
  if (!isAddress(address) || !isAddress(ctokenaddress)) {
    throw Error(
      "Invalid 'address' or 'tokenAddress' or 'spenderAddress' parameter" +
      `'${address}' or '${ctokenaddress}'.`
    )
  }
  return getContract(ctokenaddress, CTOKEN_ABI, library).getAccountSnapshot(address)
}

export async function getCurrentRate(nftaddress, library) {
  if (!isAddress(nftaddress)) {
    throw Error(
      "Invalid 'address' or 'tokenAddress'  parameter" +
      `'${nftaddress}'.`
    )
  }
  return getContract(nftaddress, NFT_MARKET_ABI, library).getCurrentRate()
}
...
export function amountFormatter(amount, baseDecimals = 18, displayDecimals = 3, useLessThan = true) {
  ...
  // amount > 0
    ...
    if (amount < minimumDisplayAmount) {
      return useLessThan
